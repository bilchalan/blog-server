...
export class PostService {
  ...
  async create(createPostDto: CreatePostDto,currentUser:User):Promise<Post> {
    ...
    const category=await this.categoryService.findOne(new ObjectId(createPostDto.category));

    const tags:string[]=[];
    if(createPostDto.tags.length){
      for(let t:number=0;t<createPostDto.tags.length;t++){
        let tag=await this.tagService.findOne(new ObjectId(createPostDto.tags[t]));
  ...
  async findAll(query:any):Promise<{filteredPostCount:number,posts:Post[],limit:number}> {
    let limit:number;
    query.limit?(limit=query.limit):(limit=2);
  ...
  async findOne(id: ObjectId):Promise<Post> {
    const post=await this.postModel.findOne({_id:id,approved:true})
    .populate('author','_id name avatar')
    .populate('category', '_id title')
    .populate({path:'tags',model:'Tag',select:'_id title'})
    .exec();
  ...
  async likes(postId:ObjectId,currentUser:User):Promise<string[]>{
    let post=await this.postModel.findOne({_id:postId,approved:true}).exec();
    if(!post) throw new NotFoundException('Post not found.');
  ...
  async findOneAndChangeCommentCount(id:ObjectId,comment:boolean):Promise<Post>{
